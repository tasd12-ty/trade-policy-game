...
echo "[run-new] rounds=${ROUNDS}, k=${K}, mode=${MODE}, countries=${COUNTRIES}"
...
echo "[done-new] CSV -> ${OUT_CSV}"
echo "[done-new] Plots (legacy+agent) -> ${RESULTS_DIR}"
echo "[done-new] New viz -> ${RESULTS_DIR}/${OUT_TAG}_summary_agent.png"
echo "[done-new] Agent logs -> ${AGENT_LOG_DIR}"
