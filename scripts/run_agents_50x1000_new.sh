...
# [new] 额外输出“智能体视角（裁剪 warmup+决策标注）”图：*_summary_agent.png
...
echo "[运行-new] 行动轮数=${ROUNDS}, 每轮仿真步数=${K}, 模式=${MODE}, 国家=${COUNTRIES}"
...
fi sk-kimi-bSf2uiJeymNFuj9lcg7z5CQNerFyCa7xz56U9iAipH7ekB9ajotqkPwXcWsjo58d
...
echo "[完成-new] CSV 输出 -> ${OUT_CSV}"
echo "[完成-new] 图表(含旧/新) -> ${RESULTS_DIR}"
echo "[完成-new] 新可视化 -> ${RESULTS_DIR}/${OUT_TAG}_summary_agent.png"
echo "[完成-new] 智能体日志 -> ${AGENT_LOG_DIR}"
