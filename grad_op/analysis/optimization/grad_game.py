...
# io-final loader
IO_FINAL_DIR = ROOT.parent / "io-final"
if str(IO_FINAL_DIR) not in sys.path:
    sys.path.insert(0, str(IO_FINAL_DIR))
...
class ParamsSource:
    ...
    type="symmetric": 使用内置对称6部门测试参数（默认）
    type="io_final":  从 io-final/ 目录加载真实IO参数
    """
    type: Literal["symmetric", "io_final"] = "symmetric"
    home_id: str = "CN2017"       # io_final 模式下 H 国目录名
    foreign_id: str = "US2017"    # io_final 模式下 F 国目录名
    tradable_sectors: Optional[List[int]] = None  # None = 全部可贸易
...
class GameConfig:
    ...
    - `params_source`：参数来源，"symmetric" 用内置测试参数，"io_final" 用真实IO数据。
    """
...
def _optimize_simultaneous_gradient_play(
    ...
    *,
    ...
    for _ in range(int(config.opt_config.iterations)):
        ...
        # === 新增：梯度裁剪 ===
        # 将梯度的范数限制在 1.0 以内，强行截断过大的梯度
        torch.nn.utils.clip_grad_norm_([tau_H_param, quota_H_param, tau_F_param, quota_F_param], max_norm=1.0) 
        # =====================
...
def _optimize_single_side_gradient(
    ...
    *,
    ...
    """单边梯度优化（Best Response）：固定对手策略，优化自己。"""
    objective = DifferentiableObjective(config.objective)
    lookahead = int(config.lookahead_periods)
    n_sectors = int(sim.params.home.alpha.shape[0])
    K = max(int(config.opt_config.multi_start), 1)
    
    opponent = "F" if target_country == "H" else "H"
    ...
    if K == 1:
        ...
        # K=1 时保持 1-D (n,)，避免 (1,n) 传播导致 import_prices 索引错误
        ...
        # 互惠约束下界（基于对手**上一轮**策略 prev_opp_tau）
        def _lb(prev_opp: torch.Tensor) -> torch.Tensor:
            if float(config.constraints.reciprocal_coeff) <= 0:
                return torch.zeros_like(prev_opp, device=DEFAULT_DEVICE).view(1, -1).repeat(K, 1)
            lb = float(config.constraints.reciprocal_coeff) * prev_opp.to(DEFAULT_DEVICE).view(1, -1).repeat(K, 1)
        ...
        strat = config.opt_config.start_strategy
        if strat == "random":
            tau_param = lb_self + (max_t - lb_self) * torch.rand((K, n_sectors), generator=gen, device=DEFAULT_DEVICE, dtype=TORCH_DTYPE)
            quota_param = min_q + (1.0 - min_q) * torch.rand((K, n_sectors), generator=gen, device=DEFAULT_DEVICE, dtype=TORCH_DTYPE)
        elif strat == "noisy_current":
            sigma = float(max(config.opt_config.start_noise, 0.0))
            tau_width = torch.clamp(max_t - lb_self, min=1e-6)
            tau_param = torch.clamp(
                tau_param + sigma * tau_width * _randn_like(tau_param, generator=gen), min=lb_self, max=ub_self
            )
            q_width = max(1.0 - min_q, 1e-6)
            quota_param = torch.clamp(quota_param + sigma * q_width * _randn_like(quota_param, generator=gen), min=min_q, max=1.0)
        elif strat == "current":
            pass
        else:
            raise ValueError(f"Unknown start_strategy: {strat}")
    ...
    lb_self_vec = lower_bound_tau(prev_opp_tau) # (n,)
...
def _optimize_static_best_response(
    ...
    *,
    ...
    """分别计算两国的 Static Best Response (Nash Equilibrium 的单步近似)。"""
    ...
    info = {"J_H_pred": J_H, "J_F_pred": J_F}
    return policies, info


def _load_params(source: ParamsSource) -> Dict:
    """根据配置加载参数。"""
    if source.type == "symmetric":
        return create_symmetric_parameters()
    elif source.type == "io_final":
...
def run_grad_experiment(config: GameConfig) -> TwoCountryDynamicSimulator:
    ...
    logger.info(f"  params_source: {config.params_source.type}")
    params_raw = _load_params(config.params_source)
    ...
    for r in range(int(config.rounds)):
        ...
        # 改用静态最优响应（类似 llm_game 的逻辑），避免同步演化的激进竞争
        policies, pred = _optimize_static_best_response(sim, config=config, prev_tau_H=prev_tau_H, prev_tau_F=prev_tau_F)
...
if __name__ == "__main__":
    # ═══════════════════════════════════════════════════════════
    # 选择参数来源:
    #   ParamsSource(type="symmetric")                           — 内置6部门对称参数
    #   ParamsSource(type="io_final", home_id="CN2017", foreign_id="US2017")  — 真实IO数据
    # ═══════════════════════════════════════════════════════════
    cfg = GameConfig(
        ...
        plot=True,
        output_dir="results_grad_10_12_F_test",
    )
