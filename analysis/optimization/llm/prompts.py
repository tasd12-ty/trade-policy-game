...
### 本国 ({country}) 经济指标
- 国民收入: {income:.4f}
- 贸易差额: {trade_balance:.4f}
- 平均物价指数: {price_mean:.4f}
- 当前关税: {current_tariff}
- 当前出口配额: {current_quota}

### 对手国 ({opponent}) 信息
- 国民收入: {opponent_income:.4f}
- 贸易差额: {opponent_trade_balance:.4f}
...
{sector_rows}"""
    ...
    def _format_sector_data(cls, sectors: List[SectorContext], active_sectors: List[int]) -> str:
        ...
        for s in sectors:
            ...
            rows.append(
                f"| {s.sector_id}{marker} | {s.price:.3f} | {s.output:.3f} | "
                f"{s.demand:.3f} | {gap_sign}{s.supply_demand_gap:.3f} | "
                f"{s.import_volume:.3f} | {s.export_volume:.3f} |"
            )
