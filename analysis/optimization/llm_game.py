...
运行：
    # 使用 Mock LLM（测试）
    python analysis/optimization/llm_game.py --mock
    ...
    python analysis/optimization/llm_game.py --preset deepseek --model deepseek-chat
"""
...
from analysis.optimization.llm import LLMPolicyAgent, OpenAIClient, MockLLMClient
...
class LLMGameConfig:
    ...
    use_mock: bool = False  # 使用 Mock LLM 进行测试
...
def main():
    """命令行入口。"""
    parser = argparse.ArgumentParser(description="LLM 博弈实验")
    parser.add_argument("--mock", action="store_true", help="使用 Mock LLM 进行测试")
    parser.add_argument("--model", default="qwen-plus", help="LLM 模型名称")
    parser.add_argument("--preset", default="qwen", choices=["qwen", "openai", "deepseek"], 
                        help="API 预设")
    parser.add_argument("--rounds", type=int, default=5, help="博弈回合数")
    parser.add_argument("--llm-plays", default="both", choices=["H", "F", "both"],
                        help="LLM 控制哪方")
    parser.add_argument("--non-llm-strategy", default="fixed", choices=["fixed", "gradient"],
                        help="非 LLM 一方的策略: fixed (保持不变) 或 gradient (梯度优化)")
    parser.add_argument("--temperature", type=float, default=0.7, help="LLM 温度参数")
    ...
    cfg = LLMGameConfig(
        ...
        llm_plays=args.llm_plays,
        non_llm_strategy=args.non_llm_strategy,
        opt_config=OptimizationConfig(learning_rate=0.01, iterations=50, optimizer="Adam"),
        plot=True,
